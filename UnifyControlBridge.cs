...
public sealed class UnifyControlBridge : IDisposable
{
        ...
        public bool TryFindPicture(string pictureName, int x1, int y1, int x2, int y2, double similarity, out int x, out int y, out int index)
        {
                EnsureReady();

                if (string.IsNullOrWhiteSpace(pictureName))
                {
                        throw new ArgumentException("图片名称不能为空。", nameof(pictureName));
                }

                x = 0;
                y = 0;
                var unify = _unify ?? throw new InvalidOperationException("UNIFY 尚未加载。");
                index = unify.FindPic(x1, y1, x2, y2, pictureName, "000000", similarity, 0, ref x, ref y, true);
                return index >= 0;
        }
        public bool TryFindCoolor(string Color, int x1, int y1, int x2, int y2,out int x, out int y)
        {
                EnsureReady();
                x = 0;
                y = 0;
                var unify = _unify ?? throw new InvalidOperationException("UNIFY 尚未加载。");
                return  unify.FindColor(x1, y1, x2, y2, Color, 1.0, 0, ref x, ref y, true);
